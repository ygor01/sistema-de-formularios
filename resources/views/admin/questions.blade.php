        ...
        <div class="container container_background">
            <div class="col-md-12">
                ...
                <div class="input-group">
                    ...
                    @if($user_id != '0')
                        <input class="input-question" type="text" placeholder="Responda aqui" name="answer[{{$questions->order-1}}]">
                        <input type="hidden" name="question_id[]" value="{{$questions->id}}">
                    @else
                        <input class="input-question" type="text" placeholder="Responda aqui" name="answer[{{$questions->order-1}}]" disabled>
                        <input type="hidden" name="question_id[]" value="{{$questions->id}}">
                    @endif
                </div>
                ...
                @if($user_id != '0')
                    <input class="form-check-input" type="radio" value="{{$options->title}}" name="answer[{{$questions->order-1}}]">
                @else
                    <input class="form-check-input" type="radio" value="{{$options->title}}" name="answer[{{$questions->order-1}}]" disabled>
                @endif
                ...
                <a href="newoption?form_id={{$forms->id}}&question_id={{$questions->id}}">
                   
                <div class="col-xs-4 col-md-2 div-btn-submit-option">
                    <button class="btn-submit" type="button" title="form_id = {{$form_id}} || question_id = {{$questions->id}}" >
                ...
                @foreach ($questions->options as $options)
                    ...
                    @else
                        <input class="form-check-input" type="checkbox" value="{{$options->title}}" name="answer[{{$questions->order-1}}][]" disabled>
                    @endif
                    <label class="form-check-label">{{$options->title}}</label><br>
                ...
                @if($question_count == '0')
                    ...
                    <div class="col-md-12">
                        <a href="newquestion?form_id={{$form_id}}&order={{$question_count+1}}">
                            <button class="btn-submit" type="button">
                                <span class="material-icons icon-add-question">library_add</span>
                                    Nova Questão
                            </button>
                        </a>                        
                    </div>
